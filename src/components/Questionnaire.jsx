...
const toFK = (key) => key.replace(/./g, “**”);
const fromFK = (key) => key.replace(/**/g, “.”);
...
if (q.type === “rank”) {
return q.options.map((o, i) => `[${answers[`${q.id}*rank*${i}`] || "—"}] ${o}`).join(”\n”);
}
if (q.type === “multi”) {
...
<style>{`@import url('https://fonts.googleapis.com/css2?family=Manrope:wght@500;600;700&family=Inter+Tight:wght@400;500&family=JetBrains+Mono:wght@400;500&display=swap'); *{box-sizing:border-box;margin:0;padding:0;} input,textarea{font-family:'Inter Tight',sans-serif;} input:focus,textarea:focus{outline:none;border-color:${T.accent}!important;box-shadow:0 0 0 3px rgba(37,99,235,0.12)!important;} button{cursor:pointer;font-family:inherit;} .sb-item{transition:background 0.15s;} .sb-item:hover{background:${T.cream}!important;} .tab-pill{transition:background 0.15s,border-color 0.15s;flex-shrink:0;} .tab-pill:hover{background:${T.cream}!important;} .btn-n{transition:background 0.15s,color 0.15s;} .btn-n:hover:not(:disabled){background:${T.ink}!important;color:#fff!important;} .btn-n:disabled{opacity:0.35;cursor:not-allowed;} .btn-e{transition:background 0.15s,transform 0.15s;} .btn-e:hover{background:${T.accent}!important;transform:translateY(-1px);} .btn-pdf{transition:background 0.15s,transform 0.15s,box-shadow 0.15s;} .btn-pdf:hover{background:${T.accentDark}!important;transform:translateY(-1px);box-shadow:0 8px 20px -6px rgba(37,99,235,0.4);} @keyframes fadeUp{from{opacity:0;transform:translateY(10px)}to{opacity:1;transform:translateY(0)}} .fade-up{animation:fadeUp 0.35s cubic-bezier(0.2,0.8,0.2,1) both;} @keyframes pd{0%,100%{box-shadow:0 0 0 3px rgba(16,185,129,0.2)}50%{box-shadow:0 0 0 5px rgba(16,185,129,0.1)}} .pd{animation:pd 2s infinite;} .tabs{display:flex;gap:8px;overflow-x:auto;padding:10px 16px;-webkit-overflow-scrolling:touch;scrollbar-width:none;} .tabs::-webkit-scrollbar{display:none;}`}</style>
